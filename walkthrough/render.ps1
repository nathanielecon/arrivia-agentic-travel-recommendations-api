...
$composition = Get-Content (Join-Path $walkthrough "index.html") -Raw
if ($composition -notmatch 'data-composition-id="arrivia-walkthrough"[^>]*data-duration="(?<duration>\d+)"') {
    throw "Could not read walkthrough duration from index.html."
}
$targetDuration = [double]$Matches.duration
...
try {
    ...
    & npx --yes hyperframes@0.7.60 snapshot walkthrough --at 0,5,10,55,100,155,160,164
    ...
    $probeJson = & $env:FFPROBE_PATH -v error -show_entries format=duration -of json $musicSource
    if ($LASTEXITCODE -ne 0) { throw "Quiet Systems duration probe failed." }
    $sourceDuration = [double](($probeJson | ConvertFrom-Json).format.duration)
    $tempo = $sourceDuration / $targetDuration
    $fadeOutStart = ($targetDuration - 4).ToString('0.000000', [Globalization.CultureInfo]::InvariantCulture)
    # Native AAC reports 17 ms less than its filtered PCM input because of encoder priming.
    # The pinned compensation yields an effective stream duration of exactly targetDuration.
    $audioTrimEnd = ($targetDuration + 0.017).ToString('0.000000', [Globalization.CultureInfo]::InvariantCulture)
    $audioFilter = "atempo=$($tempo.ToString('0.000000000', [Globalization.CultureInfo]::InvariantCulture)),loudnorm=I=-25:TP=-8:LRA=5,afade=t=in:st=0:d=2,afade=t=out:st=${fadeOutStart}:d=4,apad,atrim=end=${audioTrimEnd},asetpts=N/SR/TB"

    & $ffmpeg -loglevel error -y -i $musicSource -map 0:a:0 `
        -af $audioFilter `
        -t $targetDuration -ar 48000 -c:a aac -b:a 128k `
    ...
    & python walkthrough/verify_media.py --media $output --expected-duration $targetDuration --ffprobe $env:FFPROBE_PATH --ffmpeg $ffmpeg
    if ($LASTEXITCODE -ne 0) { throw "Final media verification failed." }
} finally {
    Pop-Location
